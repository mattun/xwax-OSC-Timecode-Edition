...
#define OSC_REFRESH 33  // 33 milliseconds
...
#define EVENT_OSC_SEND (SDL_USEREVENT + 4)
...
static void osc_time_send(struct deck *deck)
{
    struct player *pl;
    pl = &deck->player;
    ...
    //remain = player_get_remain(pl) * 1000;
    //draw_clock(surface, &lower, -remain);

    if (lo_send(osc_connection, "/xwax/timestamp", "i", elapse) == -1) {
        fprintf(stderr, "Failed to send OSC message.\n");
    }
...
static Uint32 send_osc_timer(Uint32 interval, void *param) {
    push_event(EVENT_OSC_SEND);
    return interval;
...
{
    osc_connection = lo_address_new(osc_ip, osc_port);
    bool library_update, decks_update, status_update,osc_update;

    SDL_Event event;
    SDL_TimerID timer;
    SDL_TimerID osc_timer;
    SDL_Surface *surface;
    ...
    osc_timer = SDL_AddTimer(OSC_REFRESH, send_osc_timer, NULL);
